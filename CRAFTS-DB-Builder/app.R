...
server <- function(input, output, session) {
  ...
  observe({
    req(input$folder_one,input$Master_file)
    ...
    if (length(missing_codes) > 0) {
      showNotification(
        paste("The following spectra in folder 1 do not have metadata in the metadata: ",
    ...
    if (length(missing_files) > 0) {
      showNotification(
        paste("The following codes with metadata in the master file do not have spectra in folder one: ",
